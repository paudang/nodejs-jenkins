...
export class KafkaService {
  ...
  async connect(retries = 10) {
    ...
    this.connectionPromise = (async () => {
      ...
      const { WelcomeEmailConsumer } =
        await import('@/interfaces/messaging/consumers/instances/welcomeEmailConsumer');
      while (attempt < retries) {
        try {
          ...
          try {
            ...
            logger.warn(
              `[Kafka] Could not load WelcomeEmailConsumer, using fallback: ${(e as Error).message}`,
            );
            await this.consumer.subscribe({ topic: 'user-topic', fromBeginning: true });
            await this.consumer.run({
              eachMessage: async ({ message }: EachMessagePayload) => {
                logger.info(
                  `[Kafka] Consumer: Received message on user-topic: ${message.value?.toString()}`,
                );
              },
            });
          }
